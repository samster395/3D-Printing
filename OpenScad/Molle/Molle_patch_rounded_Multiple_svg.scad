...
//for (idx = [ 0 : len(svg_files) - 1 ] ) {
//echo(svg_files[idx][1]);
//}
...
difference(){
    ...
    if(svg){
        for (idx = [ 0 : len(svg_files) - 1 ] ) {
            translate([svg_files[idx][2], svg_files[idx][3], svg_files[idx][4]]) {
                svg(depth, svg_files[idx][0], svg_files[idx][1]);
            }
        }
    }
    }
