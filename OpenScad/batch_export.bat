...
IF NOT EXIST Exported_STLs mkdir Exported_STLs

set /p file= File Name: 
echo Input is: %file%

set /p Ofile= Output file name: 
...
FOR /L %%G IN (1,1,%Nparts%) DO (
	...
	CALL :ProcessPart %%G, %file%.scad, %Ofile%-%%G
)
...
EXIT /B %ERRORLEVEL%
:ProcessPart
"C:\Program Files\OpenSCAD\openscad.exe" -o Exported_STLs\%~3.stl -D part=%~1; %~2 
